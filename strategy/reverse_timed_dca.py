...
class ReverseTimedDCA(Strategy):
    name = 'Reverse Timed DCA'

    # same as Timed DCA, except reverses the action
    # withhold portion of cash when market is below threshold, then go all in when it's not 
    def __init__(
            self, 
            budget, 
            years: int,
